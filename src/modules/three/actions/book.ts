...
import { ShakeCamera } from "@/modules/chapterx/objects/shakeCamera";
...
export default (game: GameMap) => {
    ...
    return {
        async initWidthCanvas(canvas: HTMLCanvasElement) {
            ...
            const _shake = new ShakeCamera(camera);
            ...
            const butterflies = await game.ctx.chapterx.actions.loadCity();
            ...
            function animate() {
                ...
                const e = _clock.getDelta(), n = _clock.getElapsedTime();

                _shake.update(n, e)

                butterflies && butterflies.update(e);
